...
from PyQt5.QtCore import (
    Qt, QTimer, pyqtProperty, QRectF, QThread, pyqtSignal
)
...
from PyQt5.QtGui import (
    QPainter, QColor, QFont, QRadialGradient, QLinearGradient, QPen
)
...
##############################################################################
#      VOICE ASSISTANT WIDGET WITH THREE ANIMATION STATES
##############################################################################
class VoiceAssistantWidget(QWidget):
    """
    This widget displays three states:
      - "idle": A soft, glowing static circle.
      - "listening": A breathing bubble with subtle ripple effects.
      - "responding": An audio visualization with 12 vertical bars.
    
    It updates every ~150ms.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedSize(200, 200)
        self.voice_state = "idle"  # "idle", "listening", or "responding"
        self._scaleFactor = 1.0    # For idle and listening breathing effect
        self.ripple_phase = 0      # For ripple animation in listening state
        self.num_bars = 12         # Number of vertical bars in responding state
...
##############################################################################
#              VOICE LISTENER THREAD (Single-Shot Version)
##############################################################################
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        # Voice mode widget (hidden until activated)
        self.voice_mode_widget = QWidget()
        voice_layout = QVBoxLayout(self.voice_mode_widget)
        ...
        voice_layout.addWidget(self.voice_assistant_widget, alignment=Qt.AlignCenter)
        voice_layout.addLayout(controls_layout)
        self.voice_mode_widget.setVisible(False)
        content_layout.addWidget(self.voice_mode_widget)
    ...
    def setupConnections(self):
        ...
        self.start_btn.clicked.connect(self.start_continuous_listening)
    ...
    def enter_voice_mode(self):
        ...
        self.voice_mode_widget.setVisible(True)
    ...
    def process_voice_command(self, command):
        # Process every captured command without duplicate checks
        ...
        response = self.assistant.process_command(command, self.add_log, speak_response=False)
        if response:
            self.add_message(response, is_nova=True)
